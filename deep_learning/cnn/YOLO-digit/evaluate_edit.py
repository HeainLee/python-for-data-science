...
# 스크립트의 역할 : 검증 이미지에 대한 다음 2가지 결과 수행(config.sjon파일에 기반한 검증을 수행한다.)
#     1) 검증 결과 출력 ("fscore", "precision", "recall")
#     2) 검증이미지의 결과를 'write_dname' 폴더에 저장
...
DEFAULT_CONFIG_FILE = os.path.join(yolo.PROJECT_ROOT, "saved_folder", "from_scratch_vgg.json")
DEFAULT_WEIGHT_FILE = os.path.join(yolo.PROJECT_ROOT, "saved_folder", "weights_0002.h5")
